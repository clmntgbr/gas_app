...
class ScalePainter extends CustomPainter {
  ...
  void paint(ui.Canvas canvas, ui.Size size) {
    ...
    final paddingLeft = padding == null ? 0.0 : padding!.left + sizeForStartEnd / 2;
...
LatLng calculateEndingGlobalCoordinates(LatLng start, double startBearing, double distance) {
  ...
  for (;;) {
    ...
    deltaSigma = B *
        sinSigma *
        (cosSigmaM2 +
            (B / 4.0) * (cosSignma * (-1 + 2 * cos2SigmaM2) - (B / 6.0) * cosSigmaM2 * (-3 + 4 * sinSigma * sinSigma) * (-3 + 4 * cos2SigmaM2)));
  ...
  // eq. 8
  final phi2 =
      atan2(sinU1 * cosSigma + cosU1 * sinSigma * cosAlpha1, (1.0 - f) * sqrt(sin2Alpha + pow(sinU1 * sinSigma - cosU1 * cosSigma * cosAlpha1, 2.0)));
  ...
  final lambda = atan2(sinSigma * sinAlpha1, (cosU1 * cosSigma - sinU1 * sinSigma * cosAlpha1));
